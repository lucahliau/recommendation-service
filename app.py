...
# Log helper to flush immediately.
...
def get_products_dataframe(posts, model):
    ...
    for post in posts:
        try:
            ...
            embedding = post.get('embedding', None)
            if embedding is None:
                log_info(f"Computing embedding for post {post_id} because none was provided.")
                embedding = model.encode(description).tolist()
            else:
                if isinstance(embedding, str):
                    try:
                        ...
                        log_error(f"Error converting embedding for post {post_id}: {e}")
                        embedding = model.encode(description).tolist()
            data_list.append({
                "id": post_id,
                "description": description,
            ...
            log_error(f"Error processing post {post}: {e}")
            continue
    try:
        df = pd.DataFrame(data_list)
        log_info(f"Created DataFrame with {len(df)} posts.")
        return df
    except Exception as e:
...
def recommend_products(user_liked_centers, user_disliked_centers, products_df, top_n=30, dislike_weight=1.0):
    ...
    for idx, row in products_df.iterrows():
        try:
            ...
            log_error(f"Error scoring product at index {idx}: {e}")
            scores.append(-9999)
    products_df["final_score"] = scores
    recommended = products_df.sort_values("final_score", ascending=False)
    log_info("Computed recommendations successfully.")
    return recommended.head(top_n)

# Endpoint for recommendations.
@app.route('/recommend', methods=['POST'])
def recommend_endpoint():
    log_info("Received request at /recommend endpoint.")
    try:
        data = request.get_json(force=True)
        log_info(f"Request JSON: {data}")
    except Exception as e:
        log_error(f"Error parsing JSON input in /recommend: {e}")
        return jsonify({"error": "Invalid JSON input", "details": str(e)}), 400
    ...
    try:
        user_liked_centers = np.array(liked_clusters)
        user_disliked_centers = np.array(disliked_clusters) if disliked_clusters else None
        log_info("Converted clusters to numpy arrays.")
    except Exception as e:
        log_error(f"Error converting clusters: {e}")
        return jsonify({"error": "Error processing clusters", "details": str(e)}), 500

    log_info("Initializing SentenceTransformer model for recommendation...")
    try:
        model = SentenceTransformer('all-MiniLM-L6-v2')
        log_info("Model loaded successfully for recommendation.")
    except Exception as e:
        log_error(f"Error loading model in /recommend: {e}")
        return jsonify({"error": "Error loading model", "details": str(e)}), 500

    try:
        products_df = get_products_dataframe(posts, model)
    except Exception as e:
        log_error(f"Error building products DataFrame: {e}")
        return jsonify({"error": "Error building products DataFrame", "details": str(e)}), 500

    try:
        recommendations = recommend_products(user_liked_centers, user_disliked_centers, products_df)
    except Exception as e:
        log_error(f"Error computing recommendations: {e}")
        return jsonify({"error": "Error computing recommendations", "details": str(e)}), 500

    try:
        rec_list = recommendations[['id', 'description', 'final_score']].to_dict(orient='records')
        log_info(f"Prepared output with {len(rec_list)} recommended posts.")
    except Exception as e:
        log_error(f"Error preparing output: {e}")
        return jsonify({"error": "Error preparing output", "details": str(e)}), 500

    output_json = json.dumps(rec_list)
    log_info(f"Final output JSON: {output_json}")
    sys.stdout.flush()
    return jsonify(rec_list), 200
...
if __name__ == '__main__':
    ...
    log_info(f"Starting recommendation service on port {port}")
